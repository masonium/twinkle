...
MarchingTetrahedra::MarchingTetrahedra(ImplicitEvalFunc f, const bounds::AABB& box, size_t div)
  : _bbox(box), _pps(div+1), _inv_div(1.0/div),  _field_samples(_pps*_pps*_pps),
...
void MarchingTetrahedra::precompute_grid_vertices(ImplicitEvalFunc f)
{
  ...
  {
    scalar z = lerp(_bbox.min().z, _bbox.max().z, iz * _inv_div);
    for (size_t iy = 0; iy < _pps; ++iy)
    {
      scalar y = lerp(_bbox.min().y, _bbox.max().y, iy * _inv_div);
      for (size_t ix = 0; ix < _pps; ++ix, ++i)
      {
        scalar x = lerp(_bbox.min().x, _bbox.max().x, ix * _inv_div);
        Vec3 v{x, y, z};
...
void MarchingTetrahedra::build_vertex_and_map(vector<Vertex>& vs, EdgeVertMap& em) const
{
  size_t num_verts = 0;

  for (const auto& tri: _tris)
  {
    for (const auto idx: tri.v)
    {
      // If we don't already have this edge
      if (em.find(idx) == em.end())
      {
        // add it to the map
        em.emplace(idx, num_verts);

        // Add the physical vertex to the list
        Vertex v;
        v.position = get_edge_vertex_position(idx);
        vs.push_back(v);
        ++num_verts;
      }
    }
...
Vec3 MarchingTetrahedra::get_vertex_position(size_t vi) const
{
  ...
  auto v = lerp(_bbox.min(), _bbox.max(), Vec3(cx, cy, cz) * _inv_div);
  return v;
}
...
Vec3 MarchingTetrahedra::get_edge_vertex_position(size_t e) const
{
  ...
  scalar f0 = _field_samples[vi0], f1 = _field_samples[vi1];

  assert(f0 * f1 <= 0);

  // linear interpolate between the sampled points to find the zero-crossing
  scalar zero_t = f0 == f1 ? 0.5 : f0 / (f0 - f1);
...
vector<Triangle> MarchingTetrahedra::remap_triangles(const EdgeVertMap& em) const
{
  vector<Triangle> res;
  for (const auto& tri: _tris)
  {
    res.emplace_back(em.find(tri.v[0])->second,
                     em.find(tri.v[1])->second,
                     em.find(tri.v[2])->second);
  }
  return res;
}

RawModel MarchingTetrahedra::model() const
{
  // map the tris to a contiguous set, and build the actual vertices.
  vector<Vertex> verts;
  EdgeVertMap edge_map;

  build_vertex_and_map(verts, edge_map);

  auto tris = remap_triangles(edge_map);

  RawModel m;
  assert(m.load_raw_model(verts, tris, false, false).success);
